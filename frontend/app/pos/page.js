...
export default function PosPage() {
  ...
  function finishPayment(printReceipt) {
    ...
    if (!completed) return;
    if (printReceipt) {
      router.push('/receipt/' + completed.id + '?print=1');
      return;
    }
    ...
    <div className="pos-standalone">
      ...
      <div className="pos-workspace">
        ...
        <section className="panel catalog" aria-busy={loadingStore}>
          ...
          <div className="section-heading"><div><h2>Pilih Produk</h2><p>{filteredProducts.length} / {products.length} produk • ketik untuk cari</p></div></div>
          <div style={{ display: 'flex', gap: '.5rem', marginBottom: '.75rem' }}>
            <input aria-label="Cari nama, SKU, atau barcode" style={{ flex: 1, minHeight: 44 }} placeholder="Cari nama / SKU / barcode" value={search} onChange={(e) => setSearch(e.target.value)} />
            {search && <button type="button" className="small secondary" onClick={() => setSearch('')}>Hapus pencarian</button>}
          </div>
          <div className="catalog-grid">{filteredProducts.map((product) => { const hasVariants = Number(product.variant_count) > 0; const availableStock = hasVariants ? Number(product.variant_stock_total || 0) : Number(product.stock || 0); return <button type="button" className="product-tile" key={product.id} onClick={() => chooseProduct(product)} disabled={loadingStore} aria-label={`${product.name}, ${rupiah(product.price)}, stok ${availableStock}`}>{product.photo_path ? <img src={mediaUrl(product.photo_path)} alt="" loading="lazy" /> : <span className="pos-photo-placeholder">Tanpa foto</span>}<strong>{product.name}</strong><span>{rupiah(product.price)}</span><small>{hasVariants ? `${product.variant_count} warna · Stok: ${availableStock}` : `Stok: ${availableStock}`}</small></button>; })}{!filteredProducts.length && <p className="muted">Tidak ada produk cocok "{search}"</p>}</div>
        </section>
        <aside className="panel cart"><div className="cart-mobile-header"><h2>Keranjang</h2><button type="button" className="cart-mobile-close" onClick={() => setMobileCartOpen(false)} aria-label="Tutup panel keranjang">×</button></div><label className="pos-customer-select"><span>Pelanggan</span><select value={customerId} onChange={changeCustomer} disabled={loadingStore}><option value="">Umum / Reguler</option>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.name} {customer.phone ? `(${customer.phone})` : ''}{tiersEnabled ? ` — ${tierLabel[customer.price_tier || 'retail']}` : ''}</option>)}</select></label>{cart.length > 0 && tiersEnabled && <div className="pos-tier-badge"><span className={`tier-badge ${effectiveTier}`}>{tierLabel[effectiveTier]}</span><small>{priceTier === 'retail' ? 'Harga mengikuti qty otomatis' : 'Harga mengikuti tipe pelanggan'}</small></div>}{cart.length ? <div className="cart-lines">{cart.map((item) => <article key={item.cart_id}><div className="cart-item-info"><strong>{item.name}</strong><span>{item.variant_color ? item.variant_color : ''}</span>{item.quantity > 1 && <small className="cart-line-total">{rupiah(cartSubtotal(item))}</small>}</div><div className="quantity"><button type="button" onClick={() => quantity(item.cart_id, item.quantity - 1)}>−</button><input type="number" min="1" value={item.quantity} onChange={(event) => quantity(item.cart_id, Number(event.target.value) || 1)} aria-label={`Jumlah ${item.name}${item.variant_color ? ` ${item.variant_color}` : ''}`} /><button type="button" onClick={() => quantity(item.cart_id, item.quantity + 1)}>+</button></div><label className="price-override cart-unit-price"><span className="price-prefix">Rp</span><input type="number" min="0" value={item.price_override ?? Math.round(cartPrice(item))} onChange={(event) => setPriceOverride(item.cart_id, event.target.value)} aria-label={`Harga satuan ${item.name}${item.variant_color ? ` ${item.variant_color}` : ''}`} /></label></article>)}</div> : <p>Keranjang masih kosong.</p>}<label>Kode promo<div className="promo-entry"><input value={promoCode} onChange={(event) => setPromoCode(event.target.value.toUpperCase())} placeholder="Contoh: HEMAT10" /><button type="button" onClick={applyPromo} disabled={loadingStore || !cart.length || !promoCode}>Pakai</button></div></label>{promo && <p className="muted">{promo.name}: -{rupiah(discount)} <button type="button" onClick={() => setPromo(null)}>Hapus</button></p>}<div className="total"><span>Subtotal</span><strong>{rupiah(subtotal)}</strong></div>{promo && <div className="total"><span>Diskon</span><strong>-{rupiah(discount)}</strong></div>}<div className="total"><span>Total</span><strong>{rupiah(total)}</strong></div><fieldset className="payment-methods"><legend>Metode pembayaran</legend>{[['cash', 'Tunai'], ['qris', 'QRIS'], ['debit', 'Debit'], ['transfer', 'Transfer']].map(([value, label]) => <button type="button" className={paymentMethod === value ? 'payment-option selected' : 'payment-option'} key={value} onClick={() => setPaymentMethod(value)} aria-pressed={paymentMethod === value}>{label}</button>)}</fieldset>{paymentMethod === 'cash' ? <label>Tunai diterima<input type="number" min="0" value={cash} onChange={(event) => setCash(event.target.value)} placeholder="0" /></label> : <p>Pembayaran {paymentMethod.toUpperCase()}: <strong>{rupiah(total)}</strong></p>}<p>Kembalian: <strong>{rupiah(change)}</strong></p><button type="button" disabled={loadingStore || submitting || !cart.length} onClick={checkout}>{submitting ? 'Memproses…' : `Bayar ${paymentMethod === 'cash' ? 'Tunai' : paymentMethod.toUpperCase()}`}</button>{message && <p className="message" role="status">{message}</p>}</aside>
        </section>
      </div>
