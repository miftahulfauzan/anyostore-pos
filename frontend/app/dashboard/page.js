...
function WarehouseDashboard({ data, start, end }) {
  ...
  return <div className="warehouse-dashboard">
    ...
    <section className="warehouse-lower-grid"><section className="panel"><div className="warehouse-panel-heading"><div><h2>Aksi Cepat</h2><p>Akses pekerjaan gudang yang sering dipakai.</p></div></div><div className="warehouse-action-grid">{actionItems.map((item) => { const Icon = item.icon; return <a key={item.href} href={item.href} className={`warehouse-action tone-${item.tone}`}><Icon size={16} /><span>{item.label}</span></a>; })}</div></section><section className="panel"><div className="warehouse-panel-heading"><div><h2>Baru Masuk</h2><p>Produk dengan penerimaan terbaru.</p></div><a href="/inventory/mutation-report">Lihat semua</a></div><div className="warehouse-list">{(dashboard.recent_incoming || []).map((item) => <div key={item.sku}><span><strong>{item.sku || item.name}</strong><small>{item.name}</small></span><b className="list-positive">+{Number(item.quantity || 0).toLocaleString('id-ID')}</b></div>)}</div>{!dashboard.recent_incoming?.length && <p className="muted">Belum ada penerimaan pada periode ini.</p>}</section><section className="panel"><div className="warehouse-panel-heading"><div><h2>Stok Menipis</h2><p>Prioritas pengadaan berikutnya.</p></div><a href="/inventory">Lihat stok</a></div><div className="warehouse-list">{(dashboard.low_stock || []).map((item) => <div key={item.sku}><span><strong>{item.sku || item.name}</strong><small>{item.name}</small></span><b className="list-warning">{Number(item.total_stock || 0).toLocaleString('id-ID')}</b></div>)}{(dashboard.out_of_stock || []).slice(0, 3).map((item) => <div key={`empty-${item.sku}`}><span><strong>{item.sku || item.name}</strong><small>{item.name}</small></span><b className="list-danger">0</b></div>)}</div>{!dashboard.low_stock?.length && !dashboard.out_of_stock?.length && <p className="muted">Semua stok aman.</p>}</section></section>
  </div>;
}
