...
export default function HistoryPage() {
  ...
  useEffect(() => {
    if (!selected || !isMobile) return undefined;
      ...
      <div style={{ display: 'grid', gridTemplateColumns: selected && !isMobile ? '1.1fr .9fr' : '1fr', gap: '1rem', alignItems: 'start', maxWidth: 1400, margin: '0 auto' }}>
          ...
          <section className="panel">
            ...
            <div style={{ display: 'grid', gap: '.6rem', marginTop: '.75rem' }}>
              <div style={{ display: 'flex', gap: '.5rem', flexWrap: 'wrap' }}>
                <label className="history-search-label">Cari transaksi<input style={{ flex: '1 1 200px' }} placeholder="Invoice, kasir, atau pelanggan" value={filters.search} onChange={(e) => setFilters({ ...filters, search: e.target.value })} onKeyDown={(e) => e.key === 'Enter' && applyFilter()} /></label>
          ...
          <section className="panel">
              ...
              {transactions.map((t) => (
                <article key={t.id} role="button" tabIndex={0} onClick={() => inspect(t.id)} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); inspect(t.id); } }} style={{ cursor: 'pointer', border: selected?.id === t.id ? '1px solid #2563eb' : undefined, borderRadius: 6, padding: '.6rem', background: selected?.id === t.id ? '#eff6ff' : undefined }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <span><span className={'status ' + (t.status === 'completed' ? 'paid' : t.status === 'cancelled' ? 'pending' : 'approved')}>{labelFor(statusLabels, t.status)}</span> • {labelFor(paymentLabels, t.payment_method)}</span>
                  </div>
                </article>
              ))}
            ...
            {totalPages > 1 && (
              <div style={{ display: 'flex', gap: '.5rem', justifyContent: 'center', marginTop: '1rem' }}>
                <button type="button" disabled={page <= 1} onClick={() => load(page - 1)} className="small secondary">Sebelumnya</button>
                <span className="muted" style={{ alignSelf: 'center', fontSize: '.85rem' }}>Hal {page} / {totalPages}</span>
                <button type="button" disabled={page >= totalPages} onClick={() => load(page + 1)} className="small secondary">Berikutnya</button>
              </div>
            )}
          </section>
        </div>
        ...
        {selected && (
          <div onClick={isMobile ? () => setSelected(null) : undefined} style={isMobile ? { position: 'fixed', inset: 0, background: 'rgba(15,23,42,.55)', zIndex: 100, overflow: 'auto', padding: 12, WebkitOverflowScrolling: 'touch' } : { position: 'sticky', top: '1rem', display: 'grid', gap: '1rem' }}>
            <div ref={detailRef} tabIndex={-1} role={isMobile ? 'dialog' : undefined} aria-modal={isMobile ? 'true' : undefined} aria-labelledby={isMobile ? 'transaction-detail-title' : undefined} onClick={(e) => e.stopPropagation()} style={isMobile ? { maxWidth: 720, margin: '0 auto', display: 'grid', gap: '1rem' } : { display: 'grid', gap: '1rem' }}>
              <section className="panel">
                <div className="section-heading">
                  <div>
                    <h2 id={isMobile ? 'transaction-detail-title' : undefined} style={{ fontSize: '1.1rem' }}>{selected.invoice_no}</h2>
                    <p>{new Date(selected.created_at).toLocaleString('id-ID')} • {labelFor(paymentLabels, selected.payment_method)} • {rp(selected.grand_total)}{selected.cancelled_amount ? ` • refund ${rp(selected.cancelled_amount)}` : ''}</p>
                  </div>
                  <button type="button" className="small secondary" data-dialog-close onClick={() => setSelected(null)}>Tutup</button>
                </div>

              <div style={{ display: 'flex', gap: '.4rem', marginTop: '.75rem' }}>
                <button type="button" className={activeTab === 'info' ? 'small' : 'small secondary'} onClick={() => setActiveTab('info')}>Info</button>
                <button type="button" className={activeTab === 'retur' ? 'small' : 'small secondary'} onClick={() => setActiveTab('retur')}>Retur</button>
                {canCancel && <button type="button" className={activeTab === 'cancel' ? 'small' : 'small secondary'} onClick={() => setActiveTab('cancel')}>Batal</button>}
              ...
              <div style={{ marginTop: '1rem' }}>
                {activeTab === 'info' && (
                  <div style={{ display: 'grid', gap: '.5rem' }}>
                    <p className="muted" style={{ margin: 0 }}>Klik Cetak ulang untuk struk. {selected.status !== 'completed' ? `Status: ${labelFor(statusLabels, selected.status)}. Alasan: ${selected.cancel_reason || '-'}` : ''}</p>
                ...
                {activeTab === 'retur' && (
                  <div style={{ display: 'grid', gap: '.6rem' }}>
                      ...
                      return (
                        <div key={item.transaction_item_id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', border: '1px solid var(--border)', borderRadius: 8, background: qty > 0 ? '#f0fdf4' : undefined }}>
                          ...
                          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                            <button type="button" onClick={() => setQuantities({ ...quantities, [item.transaction_item_id]: Math.max(0, qty - 1) })} disabled={qty <= 0} style={{ width: 32, height: 32, borderRadius: 6, border: '1px solid #e2e8f0', background: '#fff', color: '#0f172a', fontWeight: 700, cursor: qty <= 0 ? 'default' : 'pointer', opacity: qty <= 0 ? .4 : 1 }}>−</button>
                            <input type="number" min="0" max={remaining} value={qty} aria-label={`Jumlah retur ${item.product_name}`} onChange={(e) => setQuantities({ ...quantities, [item.transaction_item_id]: Math.min(remaining, Math.max(0, Number(e.target.value))) })} style={{ width: 48, height: 36, textAlign: 'center', borderRadius: 6, border: '1px solid #e2e8f0', fontWeight: 700, fontSize: 14 }} />
                            <button type="button" onClick={() => setQuantities({ ...quantities, [item.transaction_item_id]: Math.min(remaining, qty + 1) })} disabled={qty >= remaining} style={{ width: 32, height: 32, borderRadius: 6, border: '1px solid #e2e8f0', background: '#fff', color: '#0f172a', fontWeight: 700, cursor: qty >= remaining ? 'default' : 'pointer', opacity: qty >= remaining ? .4 : 1 }}>+</button>
                          </div>
                        </div>
                      );
                    })}
                    ...
                    <button type="button" disabled={saving} onClick={createReturn} style={{ minHeight: 44 }}>{saving ? '…' : 'Buat retur'}</button>
                  </div>
                )}
                      ...
                      {selected.items.map((item) => {
                        ...
                        return (
                          <div key={item.transaction_item_id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', border: '1px solid var(--border)', borderRadius: 8, background: qty > 0 ? '#fef2f2' : undefined }}>
                            ...
                            <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                              <button type="button" onClick={() => setCancelQuantities({ ...cancelQuantities, [item.transaction_item_id]: Math.max(0, qty - 1) })} disabled={qty <= 0} style={{ width: 32, height: 32, borderRadius: 6, border: '1px solid #e2e8f0', background: '#fff', color: '#0f172a', fontWeight: 700, cursor: qty <= 0 ? 'default' : 'pointer', opacity: qty <= 0 ? .4 : 1 }}>−</button>
                              <input type="number" min="0" max={remaining} value={qty} aria-label={`Jumlah pembatalan ${item.product_name}`} onChange={(e) => setCancelQuantities({ ...cancelQuantities, [item.transaction_item_id]: Math.min(remaining, Math.max(0, Number(e.target.value))) })} style={{ width: 48, height: 36, textAlign: 'center', borderRadius: 6, border: '1px solid #e2e8f0', fontWeight: 700, fontSize: 14 }} />
                              <button type="button" onClick={() => setCancelQuantities({ ...cancelQuantities, [item.transaction_item_id]: Math.min(remaining, qty + 1) })} disabled={qty >= remaining} style={{ width: 32, height: 32, borderRadius: 6, border: '1px solid #e2e8f0', background: '#fff', color: '#0f172a', fontWeight: 700, cursor: qty >= remaining ? 'default' : 'pointer', opacity: qty >= remaining ? .4 : 1 }}>+</button>
                            </div>
                          </div>
                        );
                      })}
