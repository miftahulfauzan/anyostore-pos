...
export default function NewProductPage() {
  ...
  function chooseMany(files) {
    const incoming = Array.from(files || []).filter((f) => f.type.startsWith('image/'));
    if (!incoming.length) return;
    for (const f of incoming) { const e = validateDataUpload(f, ['image/jpeg', 'image/png', 'image/webp']); if (e) { setMessage(e); return; } }
    let p = 0;
    setImageFiles((current) => {
      const next = [...current];
      for (let i = 0; i < next.length && p < incoming.length; i++) { if (!next[i]) { next[i] = incoming[p]; p++; } }
      return next;
    });
    p = 0;
    setImagePreviews((current) => {
      const next = [...current];
      for (let i = 0; i < next.length && p < incoming.length; i++) { if (!next[i]) { next[i] = URL.createObjectURL(incoming[p]); p++; } }
      return next;
    });
    setMessage('');
  }
  ...
  function mediaDrop(event) {
    ...
    if (images.length) chooseMany(images);
    if (video) chooseVideo(video);
  }
    ...
    <AppShell title="Tambah Produk" eyebrow="KATALOG PRODUK" actions={<a className="button-link" href="/products">Kembali ke Daftar</a>}>
      <section className="form-page">
        <form className="panel product-form" onSubmit={submit}>
          ...
          <div className="form-actions"><button type="submit" disabled={saving}>{saving ? 'Menyimpan…' : 'Simpan Produk'}</button><a href="/products">Batal</a></div>
