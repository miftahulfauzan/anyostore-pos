...
export default function EditProductPage({ params }) {
  ...
  const [adjPhoto, setAdjPhoto] = useState(null); // {mediaId, path, scale, x, y} or null
    ...
    <AppShell title="Edit Produk" eyebrow="KATALOG PRODUK" actions={<a className="button-link secondary-link" href="/products">Kembali ke Daftar</a>}>
      ...
      {adjPhoto && (
        <div onClick={() => setAdjPhoto(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.7)', display: 'grid', placeItems: 'center', zIndex: 100, padding: 20 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: '#fff', borderRadius: 12, padding: 20, maxWidth: 600, width: '100%', display: 'grid', gap: 14 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <strong>Atur Foto</strong>
              <button onClick={() => setAdjPhoto(null)} style={{ border: 'none', background: 'none', fontSize: 20, cursor: 'pointer' }}>×</button>
            </div>
            <div style={{ width: '100%', aspectRatio: '1/1', overflow: 'hidden', borderRadius: 8, background: '#f1f5f9', position: 'relative' }}>
            ...
            <div style={{ display: 'grid', gap: 8 }}>
              <label style={{ fontSize: 12 }}>Zoom: {adjPhoto.scale.toFixed(1)}× <input type="range" min="0.3" max="3" step="0.1" value={adjPhoto.scale} onChange={(e) => setAdjPhoto({ ...adjPhoto, scale: Number(e.target.value) })} style={{ width: '100%' }} /></label>
              <label style={{ fontSize: 12 }}>Geser X: {adjPhoto.x}px <input type="range" min="-300" max="300" step="1" value={adjPhoto.x} onChange={(e) => setAdjPhoto({ ...adjPhoto, x: Number(e.target.value) })} style={{ width: '100%' }} /></label>
              <label style={{ fontSize: 12 }}>Geser Y: {adjPhoto.y}px <input type="range" min="-300" max="300" step="1" value={adjPhoto.y} onChange={(e) => setAdjPhoto({ ...adjPhoto, y: Number(e.target.value) })} style={{ width: '100%' }} /></label>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={saveAdj} style={{ flex: 1, minHeight: 40, borderRadius: 8, border: 'none', background: '#1e3a5f', color: '#fff', fontWeight: 700, cursor: 'pointer' }}>Simpan</button>
              <button onClick={() => { setAdjPhoto({ ...adjPhoto, scale: 1, x: 0, y: 0 }); }} style={{ minHeight: 40, padding: '0 16px', borderRadius: 8, border: '1px solid #e2e8f0', background: '#fff', cursor: 'pointer' }}>Reset</button>
            </div>
          </div>
        </div>
      )}
