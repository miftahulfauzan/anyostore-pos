...
export default function ProductsPage() {
  ...
  return <AppShell title="Produk & Inventori" eyebrow="KATALOG PRODUK" actions={<a className="button-link" href="/products/new">Tambah Produk</a>}>
    <section className="panel catalog-panel">
      ...
      {loading ? <p>Memuat produk…</p> : <div className={`product-list ${view === 'grid' ? 'grid-view' : ''}`}>{products.map((product) => <article key={product.id} className="product-row"><div className="product-photo">{product.photo_path ? <img src={mediaUrl(product.photo_path)} alt={`Foto ${product.name}`} loading="lazy" style={product.photo_transform ? (()=>{const t=(product.photo_transform||'').split(',').map(Number); return {objectFit:'none',transform:`scale(${t[0]}) translate(${t[1]||0}px,${t[2]||0}px)`,width:'100%',height:'100%'};})():{}} /> : <span>Tanpa foto</span>}</div><div className="product-description"><strong>{product.name}</strong><span>{product.category_name} · {product.sku || 'Tanpa SKU'}</span>                  {Number(product.variant_count) > 0 && <div className="variant-summary"><span>{product.variant_count} varian</span>{String(product.variant_colors || '').split('|').filter(Boolean).slice(0, 4).map((color) => <i key={color} title={color}>{color}</i>)}</div>}<div className="product-actions"><a className="edit-link" href={`/products/${product.id}/edit`}>Kelola produk</a><button type="button" className="delete-link" onClick={async () => { if (!confirm(`Hapus "${product.name}"?`)) return; try { const r = await fetch(`${apiUrl}/products/${product.id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token()}` } }); const b = await r.json(); if (!r.ok) throw new Error(b.message); setMessage(b.data.message); load(search); } catch (e) { setMessage(e.message); } }}>Hapus</button></div></div><div><strong>Rp{Number(product.price).toLocaleString('id-ID')}</strong><span>Stok {product.stock}</span></div></article>)}{!products.length && <div className="empty-state"><strong>Produk tidak ditemukan.</strong><span>Coba kata kunci lain atau tambahkan produk baru.</span><a href="/products/new">Tambah produk</a></div>}</div>}
    </section>
  </AppShell>;
}
