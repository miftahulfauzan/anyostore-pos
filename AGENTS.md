...
- Stok (inventory_page.dart): header ala POS untuk OWNER — 1 baris 2 segmen (_InvHeaderSegment): segmen "Toko / Gudang" (kiri, label generik TANPA nama toko aktif) membuka bottom sheet pilihan toko (`_pickBranch`: daftar cabang + "Semua toko/gudang"), segmen "Cari produk" (kanan) toggle accordion field cari; TIDAK ada dropdown toko kedua di bawah. Selain owner langsung field cari saja. Ringkasan Produk/Stok/Stok rendah/Habis jadi 1 baris dibagi 4 (_StatCell, font kecil seragam 9/13 + FittedBox scaleDown), berjarak 12. Ada baris kontrol: dropdown Urut (Nama A-Z/Z-A, Stok terendah/tertinggi — client-side `_sorted`) + toggle Card/Grid bergaya pill (_ViewToggle, senada PillTabs). MODE CARD = kartu besar 1 kolom dengan foto produk (CachedNetworkImage memCacheWidth 120); MODE GRID = kartu 2 kolom dengan foto (memCacheWidth 420, childAspectRatio 0.8). Mode gelap: _StatCell pakai GlassCard (auto-dark).
