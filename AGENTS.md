...
- MODE GELAP: helper task_ui.dart `pageBg(context)` (krem #F5F1EA / gelap #12151B) & `ink(context)` (denim / #E7ECF4) dipakai di background & teks utama (history/inventory/reports/more/dashboard/products; ignore_for_file prefer_const_constructors di file tsb karena warna dinamis). GlassCard otomatis gelap dari Theme.brightness; navbar, sheet, dialog, input sudah theme-aware.
