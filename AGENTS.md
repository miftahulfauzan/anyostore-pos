...
- GlassNavBar - bottom nav Liquid Glass MELAYANG & TRANSPARAN (gaya iOS 26, diadaptasi dari QWEA0/Liquid-Glass-Android): pil mengambang dengan margin 14 kiri/kanan/bawah (ClipRRect radius 30 semua sisi), BackdropFilter blur 24 + saturasi, rim atas dengan chromatic dispersion merah/sian yang bergeser mengikuti "sumber cahaya" (AnimationController 7 detik), specular sweep di permukaan kaca, FAB tengah seperti tetesan kaca (highlight ikut cahaya). Navbar dipasang sebagai OVERLAY di body PosPage (Positioned bottom, BUKAN bottomNavigationBar) sehingga area di luar pil benar-benar transparan dan konten halaman mengalir di belakangnya. Konsekuensi: tiap tab diberi bottom padding ~104 (Kasir: grid padding bawah 178+bottomPad + tombol Keranjang di-Positioned di atas navbar, bottom 98+bottomPad). 4 ikon (Riwayat, Stok, Laporan, Lainnya) + FAB tengah ikon keranjang (tab Kasir). Mapping slot: 0->tab1, 1->tab2, 2->tab0, 3->tab3, 4->tab4. PosPage.requestTab dipakai halaman lain untuk pindah tab (mis. Dashboard -> Kasir). Widget test: test/glass_nav_bar_test.dart.
- Header (pos_page.dart) gaya INSTAGRAM: logo BrandLogo kiri (leadingWidth 52), judul "Anyostore App" + username kecil di tengah (centerTitle via theme appBarTheme), aksi logout kanan, hairline 0.5px di bawah. AUTO-HIDE tetap jalan: menghilang mulus saat konten di-scroll ke bawah (NotificationListener<ScrollNotification> di body -> _barVisible, _AutoHideAppBar pakai AnimatedContainer height kToolbarHeight+0.5->0) dan muncul lagi saat scroll ke atas. Semua AppBar halaman lain otomatis centerTitle: true.
