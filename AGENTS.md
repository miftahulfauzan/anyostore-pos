...
- STOK MASUK/KELUAR (_InOutForm + _CatalogPicker): UI ala POS (grid produk 2 kolom foto+nama+SKU/stok, cari, ketuk -> dialog qty/cost) dengan WARNA PEMBEDA: Stok Masuk HIJAU (#2E7E4F), Stok Keluar ORANYE (#C2410C) — AppBar, banner "Barang MASUK/KELUAR", tombol & header picker ikut warna. Dropdown Channel punya tombol + Tambah Channel (POST /api/inventory/channels, api_client.createChannel).
