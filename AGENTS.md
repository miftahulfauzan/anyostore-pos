...
- Stok (inventory_page.dart): ringkasan Produk/Stok/Stok rendah/Habis jadi 1 baris dibagi 4 (_StatCell, font kecil seragam 9/13 + FittedBox scaleDown), berjarak 12 dari field cari. Mode gelap: _StatCell pakai GlassCard (auto-dark).
...
- OFFLINE PENUH (riwayat + laporan + varian produk): data yang pernah dimuat otomatis tersimpan di cache lokal (sqflite anyostore_offline.db, tabel `generic_cache`, DB version 3) lewat `OfflineStore.cacheSet/cacheGet`. Saat internet putus (`ApiException.isNetwork`), halaman otomatis memakai cache: Riwayat Transaksi per filter/halaman (key `history-<dari>-<sampai>-<status>-<cari>-<page>`), tab Retur (`history-retur`), detail transaksi (`tx-detail-<id>`), Laporan per section/preset/branch (`report-<section>-<preset>-<branchId>`), detail produk + varian POS (`product-<branch>-<productId>` — di-cache setiap kali produk dibuka online). Banner kuning "Offline — memakai data tersimpan..." tampil di semua tab (PosPage, SafeArea top, konten terdorong ke bawah) lewat `OfflineStatus.offline` (ValueNotifier, di-set api_client saat request sukses/gagal jaringan). Auto-sync: pos_page listen `Connectivity().onConnectivityChanged` -> `OfflineStatus.notifyOnline()` (offline=false + `syncTick`++) -> `_syncPending()` (transaksi offline) + reload POS; HistoryTab & ReportsPage listen `OfflineStatus.syncTick` dan panggil `_load(silent:true)` supaya data normal lagi dari server. Kalau cache belum ada saat offline, muncul pesan "belum tersimpan offline".
