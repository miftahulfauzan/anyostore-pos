...
- STOK MASUK/KELUAR (_InOutForm + _CatalogPicker): UI ala POS (grid produk 2 kolom foto CachedNetworkImage memCacheWidth 420 + nama+SKU/stok, cari, ketuk -> dialog qty/cost) dengan WARNA PEMBEDA: Stok Masuk HIJAU (#2E7E4F), Stok Keluar ORANYE (#C2410C) — AppBar, banner SOLID + teks PUTIH, tombol & header picker ikut warna; warna penuh sampai PINGGIR ATAS via `extendBodyBehindAppBar:true` + `flexibleSpace: Container(color)` + `systemOverlayStyle.light`; judul AppBar DIJAMIN PUTIH via `titleTextStyle` (theme global denim). Dropdown Channel punya tombol + Tambah Channel (POST /api/inventory/channels, api_client.createChannel). `POST /inventory/outgoing` otomatis mendaftarkan channel baru ke `sales_channels` (INSERT IGNORE) supaya tabel channel satu-satunya sumber. Tabel legacy kandidat di-audit via `node scripts/audit-legacy-tables.js` (read-only).
