...
- GlassNavBar - bottom nav Liquid Glass MELAYANG (gaya iOS 26, diadaptasi dari QWEA0/Liquid-Glass-Android): pil mengambang dengan margin 14 kiri/kanan/bawah (ClipRRect radius 30 semua sisi), BackdropFilter blur 24 + saturasi, rim atas dengan chromatic dispersion merah/sian yang bergeser mengikuti "sumber cahaya" (AnimationController 7 detik), specular sweep di permukaan kaca, FAB tengah seperti tetesan kaca (highlight ikut cahaya). 4 ikon (Riwayat, Stok, Laporan, Lainnya) + FAB tengah ikon keranjang (tab Kasir). Mapping slot: 0->tab1, 1->tab2, 2->tab0, 3->tab3, 4->tab4. PosPage.requestTab dipakai halaman lain untuk pindah tab (mis. Dashboard -> Kasir). Widget test: test/glass_nav_bar_test.dart.
- AppBar utama (pos_page.dart) AUTO-HIDE: header "Anyostore App · user" menghilang mulus saat konten di-scroll ke bawah (NotificationListener<ScrollNotification> di body -> _barVisible, _AutoHideAppBar pakai AnimatedContainer height kToolbarHeight->0) dan muncul lagi saat scroll ke atas.
