...
- Menu Lainnya: grup menu (UTAMA/AKUN & TOKO/PRODUK & INVENTORI/TRANSAKSI & KEUANGAN/MANAJEMEN), tile Jenis Pelanggan (buka CustomersPage), Antrean Offline dengan badge jumlah, Riwayat Aktivitas (ActivityLogPage) dengan filter chip Semua/Transaksi/Stok/Retur/Produk (query ?action= prefix di /api/activity-logs). Owner/manager/admin punya tile "Audit Modal vs Harga" (CostAuditPage): daftar produk cost >= harga jual + total selisih (sumber data GET /products yang memuat cost). Script server: `node scripts/audit-product-cost.js`.
