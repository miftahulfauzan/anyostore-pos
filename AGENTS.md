...
- Stok (inventory_page.dart): ringkasan Produk/Stok/Stok rendah/Habis jadi 1 baris dibagi 4 (_StatCell, font kecil seragam 9/13 + FittedBox scaleDown), berjarak 12 dari field cari. Ada baris kontrol: dropdown Urut (Nama A-Z/Z-A, Stok terendah/tertinggi — client-side `_sorted`) + toggle Card/Grid (SegmentedButton, grid 2 kolom). Mode gelap: _StatCell pakai GlassCard (auto-dark).
