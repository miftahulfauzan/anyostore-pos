...
- GlassNavBar - bottom nav Liquid Glass MELAYANG & TRANSPARAN (gaya iOS 26, diadaptasi dari QWEA0/Liquid-Glass-Android): pil kaca mengambang margin 14 (radius 28), BackdropFilter blur 24 + saturasi, rim chromatic dispersion + specular sweep. TIDAK ada FAB menonjol — 5 item sebaris senada, urutan: POS (keranjang, paling kiri), Riwayat, Stok, Laporan, Lainnya; mapping slot = index tab langsung (0..4). Item aktif dapat pil latar denim transparan + teks gelap tebal (kontras tinggi), nonaktif abu #5F5F5D. Parameter `cartCount` menampilkan badge merah jumlah item keranjang aktif di item POS. Navbar dipasang sebagai OVERLAY di body PosPage (Positioned bottom, BUKAN bottomNavigationBar) sehingga area di luar pil transparan & konten mengalir di belakangnya. Konsekuensi: tiap tab bottom padding ~104 (Kasir: grid padding bawah 178+bottomPad + tombol Keranjang di-Positioned bottom 98+bottomPad, dan tombol hanya muncul kalau keranjang TIDAK kosong). PosPage.requestTab dipakai halaman lain untuk pindah tab. Widget test: test/glass_nav_bar_test.dart.
- Header (pos_page.dart) gaya INSTAGRAM: TANPA logo — leading kosong selebar 52 (penyeimbang), judul "Anyostore App" satu baris di tengah (centerTitle via theme appBarTheme), aksi logout kanan, hairline 0.5px di bawah. AUTO-HIDE tetap jalan: menghilang mulus saat konten di-scroll ke bawah (NotificationListener<ScrollNotification> di body -> _barVisible, _AutoHideAppBar pakai AnimatedContainer height kToolbarHeight+0.5->0) dan muncul lagi saat scroll ke atas. Semua AppBar halaman lain otomatis centerTitle: true.
