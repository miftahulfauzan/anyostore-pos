...
- MODE GELAP: helper task_ui.dart `pageBg(context)` (krem #F5F1EA / gelap #12151B) & `ink(context)` (denim / #E7ECF4) dipakai di background & teks utama (history/inventory/reports/more/dashboard/products; ignore_for_file prefer_const_constructors di file tsb karena warna dinamis). GlassCard otomatis gelap dari Theme.brightness; navbar (glass gelap + ikon/teks #F1F5FB), sheet, dialog, input theme-aware; filter Riwayat/Laporan (fill #1F2530, border #2A3140, fokus #7FA8CF); halaman Lainnya lengkap gelap (chip, divider, profil, keluar, versi).
