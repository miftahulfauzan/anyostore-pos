...
- UI responsif: main.dart membungkus app dengan skala berbasis design 390x844 (Transform.scale, clamp 0.8-1.35) supaya di HP kecil UI mengecil. Kotak desain di-`Align(topCenter)` (BUKAN Center) supaya warna AppBar/halaman beraksen (hijau/oranye) memenuhi area status bar — pita atas warna beda hilang; sisa tinggi muncul di BOTTOM sebagai warna tema. Area di luar area desain diisi warna tema (ColoredBox, bukan hitam) + SystemChrome edge-to-edge dengan status bar transparan (ikon gelap) — supaya TIDAK ada garis hitam di HP rasio layar lebih tinggi.
