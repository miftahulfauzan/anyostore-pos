...
- PERFORMA: grid POS & foto varian pakai `memCacheWidth` (decode gambar kecil: 420 / 120) supaya render + memori ringan; `_loadData` stale-while-revalidate — kalau ada cache produk berapa pun umurnya, langsung tampil (instant open) lalu refresh dari server di background; kalau sync hari ini, tanpa sentuh server sama sekali.
