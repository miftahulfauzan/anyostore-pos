...
- STOK MASUK/KELUAR (_InOutForm + _CatalogPicker): UI ala POS (grid produk 2 kolom foto CachedNetworkImage memCacheWidth 420 + nama+SKU/stok, cari, ketuk -> dialog qty/cost) dengan WARNA PEMBEDA: Stok Masuk HIJAU (#2E7E4F), Stok Keluar ORANYE (#C2410C) — AppBar, banner SOLID + teks PUTIH, tombol & header picker ikut warna; warna penuh sampai area status bar (Container padding.top). Dropdown Channel punya tombol + Tambah Channel (POST /api/inventory/channels, api_client.createChannel).
...
- ROLE GUDANG: navbar tab POS -> Dashboard & Riwayat -> Keuangan (DashboardPage/FinancePage di pos_page, `isGudang`).
