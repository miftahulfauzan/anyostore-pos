...
- Menu Lainnya: grup menu (UTAMA=Dashboard / AKUN & TOKO=Akun Saya, Ganti Akun, Pengaturan / PRODUK & INVENTORI=Daftar Produk, Riwayat Stok, Laporan Masuk/Keluar, Audit Modal vs Harga (owner/manager/admin) / TRANSAKSI & KEUANGAN=Jenis Pelanggan, Laci Kas, Keuangan, Komisi / MANAJEMEN=Pegawai, Promo, Riwayat Aktivitas), tile Jenis Pelanggan (buka CustomersPage), Antrean Offline dengan badge jumlah, Riwayat Aktivitas (ActivityLogPage) dengan filter chip Semua/Transaksi/Stok/Retur/Produk (query ?action= prefix di /api/activity-logs). CATATAN: menu Belum difilter per role (kasir/gudang masih melihat menu manajemen) — keputusan user masih pending. Script server: `node scripts/audit-product-cost.js`.
