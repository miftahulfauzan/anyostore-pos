...
- Dashboard (dashboard_page.dart) — REDESIGN "RINGKASAN" ala SaaS (pakai dependency fl_chart): header card (judul Ringkasan + ikon kalender, filter Date From/To + dropdown Preset Hari ini/7 Hari/30 Hari/Bulan ini/Rentang kustom + tombol Apply, label tanggal aktif "Sabtu, 15 Agustus 2026"), 3 kartu ringkasan (Masuk hijau, Keluar merah, Selisih biru — dari mutationReport in/out total_qty periode), LINE CHART "Pergerakan Stock 7 Hari" (masuk biru/keluar merah, smooth curve, titik, area fill, grid tipis, tooltip, badge total), DONUT "Status Stok" (Aman/Hampir Habis/Kosong dari stockTotal summary + ring progress), BAR CHART "Stok per Kategori" (label miring 45°, dummy), HORIZONTAL BAR "Top Produk Keluar" (magenta, dummy). Data harian digabung dari api.mutations per tanggal (qty positif=masuk, negatif=keluar). Data dummy default supaya langsung tampil.
