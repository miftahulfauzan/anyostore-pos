...
router.get('/incoming/targets', authorize('owner','manager','admin','gudang'), async (req,res,next)=>{try{const branchId=req.user.role==='owner'?null:req.user.branch_id;const [rows]=await db.execute(branchId?'SELECT id,name,type FROM branches WHERE id=? AND is_active=TRUE':'SELECT id,name,type FROM branches WHERE is_active=TRUE ORDER BY name',branchId?[branchId]:[]);res.json({success:true,data:rows});}catch(e){next(e);}});
