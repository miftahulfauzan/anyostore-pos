...
router.get('/', async (req, res, next) => {
  try {
    ...
    const salesSql =
      'SELECT ' +
      'COALESCE(SUM(CASE WHEN DATE(t.created_at + INTERVAL 7 HOUR) = DATE(UTC_TIMESTAMP() + INTERVAL 7 HOUR) THEN t.grand_total - t.cancelled_amount - t.refunded_amount ELSE 0 END), 0) AS today_sales, ' +
      ...
      'COALESCE(SUM(CASE WHEN DATE(t.created_at + INTERVAL 7 HOUR) >= DATE_SUB(DATE(UTC_TIMESTAMP() + INTERVAL 7 HOUR), INTERVAL 6 DAY) THEN t.grand_total - t.cancelled_amount - t.refunded_amount ELSE 0 END), 0) AS seven_day_sales, ' +
      'COALESCE(SUM(CASE WHEN YEAR(t.created_at + INTERVAL 7 HOUR) = YEAR(UTC_TIMESTAMP() + INTERVAL 7 HOUR) AND MONTH(t.created_at + INTERVAL 7 HOUR) = MONTH(UTC_TIMESTAMP() + INTERVAL 7 HOUR) THEN t.grand_total - t.cancelled_amount - t.refunded_amount ELSE 0 END), 0) AS month_sales ' +
    ...
    const trendSql =
      'SELECT DATE(t.created_at + INTERVAL 7 HOUR) AS date, COALESCE(SUM(t.grand_total - t.cancelled_amount - t.refunded_amount), 0) AS sales ' +
